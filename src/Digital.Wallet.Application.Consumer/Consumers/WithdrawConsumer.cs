...
public class WithdrawConsumer(ILogger<WithdrawConsumer> logger, IServiceProvider serviceProvider) : BaseConsumer(logger, serviceProvider)
{
    public override async Task HandleMessageAsync(BasicDeliverEventArgs ea, IChannel channel, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var @event = JsonSerializer.Deserialize<WithdrawEvent>(message);
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError(ex, "Erro ao processar mensagem de saque.");
        }
    }
}